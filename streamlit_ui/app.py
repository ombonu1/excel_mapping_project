...
**Step 2.** Click **“🚀 Run analysis”** – the **Understanding Agent** analyses structure & relationships.

**Step 3.** The **Mapping Agent** and **Product Selector Agent** both use that JSON:
- Mapping Agent → generates a **process map PNG**
- Product Selector Agent → suggests **products / systems / tools**
...
with st.sidebar:
    ...
    st.markdown("3️⃣ Review JSON, process map PNG & product suggestions")
    st.markdown("4️⃣ Give feedback on the map")
    st.markdown("5️⃣ Download final output")
...
# Step 3 – Mapping + Product Agents
# ------------------------------

    # 3a – Mapping Agent (PNG)
    with st.spinner("Running Mapping Agent & generating PNG…"):
        map_result = run_mapping_agent(understanding_json)

    st.session_state["map_result"] = map_result

    # 3b – Product Selector Agent
...
# DISPLAY STEP 2 OUTPUT – UNDERSTANDING AGENT
# ------------------------------------------------------------------
if "understanding_json" in st.session_state:
    st.markdown("---")
    st.subheader("Step 2 – 📄 Understanding Agent JSON output")
...
# DISPLAY STEP 3 OUTPUT – MAPPING & PRODUCT SELECTOR
# ------------------------------------------------------------------
...
with col_map:
    ...
    if "map_result" in st.session_state:
        ...
        if png_bytes:
            ...
            st.download_button(
                label="📥 Download process map PNG",
                data=png_bytes,
...
# 3b – Product Selector Agent output
with col_prod:
...
if "understanding_json" in st.session_state and "map_result" in st.session_state:
    st.markdown("---")
    st.subheader("Step 4 – Feedback on the process map")

    st.markdown(
        "Use this section to tell the Feedback Agent what you like, what’s missing, "
        "or what should change in the process map."
    )

    feedback_text = st.text_area(
        "Your feedback on the process map",
        placeholder="e.g. 'The approvals step is missing', 'Combine steps 3 and 4', 'Rename the report to…'",
        key="feedback_text",
        height=150,
    )

    if st.button("✉️ Send feedback to Feedback Agent"):
...
if (
    ...
    st.markdown("---")
    st.subheader("Step 5 – Generate final deliverable")
    ...
    if st.button("📄 Generate final output"):
        with st.spinner("Running Final Output Agent…"):
