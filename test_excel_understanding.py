...
async def run_test(excel_path: str):
    ...
    # 3. Pass the file BYTES directly, not as a dict with "excel_files" key
    # The file content will be available in the session context
    events = await runner.run_debug(
        {
            "message": f"Analyze the Excel file '{file_path.name}' and extract metadata",
            "files": {file_path.name: excel_bytes}
        }
    )
